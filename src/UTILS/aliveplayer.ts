export class PlayersAliveMonitor {
    ...
    public startObserving(callback: (newValue: string | null) => void) {
        const tryStart = () => {
            ...
            if (this.element) {
                console.log("[KxsClient - LOG] [PlayersAliveMonitor] Html element found :", this.element.outerHTML);
        ...
        if (!tryStart()) {
            console.log("[KxsClient - LOG] [PlayersAliveMonitor] Html element not found, waiting  MutationObserver...");
    ...
    private observeElement(callback: (newValue: string | null) => void) {
        ...
        console.log("[KxsClient - LOG] [PlayersAliveMonitor] Observation started on", this.element.outerHTML);
    }
    ...
    public stopObserving() {
        if (this.observer) {
            ...
            console.log("[PlayersAliveMonitor] Observation stopped.");
        }
    }
}
